...
> **Tip**: The `hosts` field in the kiam server certificate must include the value _release-name_-server, _e.g_. `my-release-server`, and also `127.0.0.1` so that liveness/readiness probes succeed.
...
Alternately, if you are generating your certificates using something like [cert-manager](https://github.com/jetstack/cert-manager) which stores certificates in Kubernetes secrets for you, use the following pattern:

```yaml
agent:
  extraSecretMounts:
    - name: cacert
      secretName: cert-manager-ca-intermediate-tls
      mountPath: /etc/kiam/cacert
  tlsConfig:
    caPath: /etc/kiam/cacert/tls.crt
    certFile: tls.crt
    keyFile: tls.key
...
and then create your certs with something like the following:
...
The `secretName` parameter must match the pattern that this chart creates by default.
...
`agent.extraSecretMounts` | Additional agent container Secret mounts | `[]`
...
`agent.tlsConfig.caPath` | Path to local file containing the agent's CA certificate(s) | `null`
`agent.tlsConfig.certFile` | Name of local file containing the agent's certificate | `cert`
`agent.tlsConfig.keyFile` | Name of local file containing the agent's certificate | `key`
...
`server.extraSecretMounts` | Additional server container Secret mounts | `[]`
...
`server.tlsConfig.caPath` | Path to local file containing the server's CA certificate(s) | `null`
`server.tlsConfig.certFile` | Name of local file containing the server's certificate | `cert`
`server.tlsConfig.keyFile` | Name of local file containing the server's certificate | `key`
